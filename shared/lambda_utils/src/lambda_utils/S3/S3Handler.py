...
class S3Handler:
    ...
    def __init__(self, record=None, bucket_name=None, key_name=None, profile_name=None, config=None):
        self.s3 = None
    ...
    def start_multipart_upload(self, metadata, content_type):
        ...
        if self.part_object is None:
            self.part_object = self.object.initiate_multipart_upload(Metadata=metadata, ContentType=content_type)
    ...
    def getS3Files(self):
        """
        Retrieve all files from a S3 bucket based on a record. This auto unzip a file if the file comes zipped
        :param record: the json of the event containint the information to where retrieve the file
        :return: a list of double elements, one containing the filename and the other the file data as a string
        """
        try:
            logger.info("Reading Bucket and key name")
            # logger.info("Bucket: %s | File: %s" % (self.getS3BucketName(), self.getS3BucketKey()))
                ...
                with io.BytesIO(obj.get()["Body"].read()) as tf:
                    ...
                    with zipfile.ZipFile(tf, mode='r') as zipf:
                        ...
                        for subfile in zipf.namelist():
                            if subfile.startswith('__MACOSX'):
                                continue
                            files.append({'file': subfile, 'data': zipf.read(subfile)})
                        return files
