...
SQS_SHOPIFY_CANCELLATIONS = os.environ.get('SQS_SHOPIFY_CANCELLATION')
...
async def process_event(event):
    ...
    if event_type == 'order.cancelled':
        if event_body.get('payload', {}).get('service_level') == 'IN_STORE_HANDOVER':
            LOGGER.info('Received a order.cancelled message with a service level of'
                        ' IN_STORE_HANDOVER. Will not process.')
            return True
        queue_list = [SQS_SHOPIFY_CANCELLATIONS]
    else:
