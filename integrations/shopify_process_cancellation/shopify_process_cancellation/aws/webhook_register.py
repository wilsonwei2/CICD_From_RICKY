...
WEBHOOK_EVENT_FILTER = [
    event.strip()
    for event in os.environ.get('WEBHOOK_EVENT_FILTER', 'order.cancelled').split(',')
]
...
async def _validate_webhook() -> str:
    ...
    try:
        ...
        if 'resource_not_found' in str(ex):
            ...
            await _create_hook(WEBHOOK_URL)
        else:
            LOGGER.error(str(ex))
    return 'Completed'
...
async def _create_hook(callback_url, name=WEBHOOK_NAME):
...
async def _restart_hook(name=WEBHOOK_NAME):
