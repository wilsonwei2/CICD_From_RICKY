...
TREAT_ALL_ORDERS_AS_HISTORICAL = bool(os.environ.get('TREAT_ALL_ORDERS_AS_HISTORICAL', False))
...
async def process_return(message):
    ...
    customer_order_envelope = Utils.get_newstore_conn().get_customer_order(ns_return['order_id'])
    if not customer_order_envelope:
        raise Exception(f'Customer order for order id {ns_return["order_id"]} not found on NewStore')
...
async def _update_payments_info_if_needed(customer_order, payments_info):
    ...
    updated_payments_info = verify_and_update_missing_payment_info(Utils.get_shopify_conn(), payments_info, customer_order)
...
async def _handle_store_order_return(customer_order, ns_return, payments_info, store_tz):
    ...
    if not (is_historical and is_historical.lower() == 'true') \
        ...
        if original_order and original_order['internalId'] != cash_sale['internalId']:
            ...
            payments_info = Utils.get_newstore_conn().get_payments(ext_order['order_uuid'])
            if not payments_info:
                raise Exception('Payment Account for original order %s not found on NewStore' % original_order.externalId)
            LOGGER.info(f'Original Order Payment Info: {json.dumps(payments_info, indent=4)}')

    return_parsed = await por.transform_order(cash_sale, ns_return, payments_info, customer_order, store_tz)
    LOGGER.info(f"Transformed return: \n{json.dumps(serialize_object(return_parsed), indent=4, default=Utils.json_serial)}")
    ...
    cash_refund['itemList'] = CashRefundItemList(cash_refund_items + payment_items)
...
async def _handle_web_order_return(customer_order, ns_return, payments_info):
    ...
    if not (is_historical and is_historical.lower() == 'true') \
        ...
        if TREAT_ALL_ORDERS_AS_HISTORICAL or ns_return.get('is_historical'):
            LOGGER.info('Return is historical. Proceed to create standalone CashRefund')
            return_parsed = await pwor.transform_web_order(customer_order, ns_return, payments_info, sales_order,
                                                           None)
        elif ns_return['return_location_id'].endswith(netsuite_config['dc_suffix']):
            LOGGER.info('Return is online, proceed to create a CashRefund from a ReturnAuthorization')
            result, return_authorizations = get_return_authorizations(sales_order.internalId)
            ...
            if not return_authorization:
                LOGGER.error('There\'s no ReturnAuthorization currently on NetSuite that matches this return.')
                return False
            return_parsed = await pwor.transform_web_order(customer_order, ns_return, payments_info, sales_order,
                                                           return_authorization)
        else:
            LOGGER.info('Return is in store, proceed to create standalone CashRefund')
            return_parsed = await pwor.transform_web_order(customer_order, ns_return, payments_info, sales_order,
                                                           None)
    else:
    ...
    return inject_web_return(return_parsed)
...
def get_return_timeline_timestamp(ns_return, customer_order):
    ...
    if ns_returned_at_date is None:
        raise Exception(f'Could not find required key returned_at in return data: {ns_return}.')
    if len(ns_returned_at_date) == 0:
        raise Exception(f'Expected item ids in newstore return but none were found: {ns_return}.')

    ns_returned_items = ns_return.get('items')
    if ns_returned_items is None or len(ns_returned_items) == 0:
        raise Exception(f'Could not find items in the return data: {ns_return}.')

    ns_returned_item_ids = list(map(lambda item: item.get('product_id'), ns_returned_items))
    ...
    if len(order_timeline) == 0:
        raise Exception(f'Expected key order_timeline in customer order to contain items to check for returns.')

    correct_returned_at_timestamp = None
    for timeline_item in order_timeline:
        timeline_returned_items = timeline_item.get('payload', {}).get('returned_items')
        if timeline_returned_items is None or len(timeline_returned_items) == 0:
            continue

        timeline_returned_item_ids = list(map(lambda x: x.get('id'), timeline_returned_items))
        if collections.Counter(timeline_returned_item_ids) != collections.Counter(ns_returned_item_ids):
            continue

        LOGGER.debug('Found order_timeline entry that matches the items in the ns_return.')
...
async def _verify_return_authorization(return_authorization, returned_product_ids):
    ...
    result, cash_refund = get_transaction_with_created_from_id(return_authorization.internalId, '_cashRefund')
    if not result:
        raise Exception('An error occurred while searching for a CashRefund created from the return authorization')

    if cash_refund:
        LOGGER.info('This return authorization was already utilized in another CashRefund, go to next')
        return False
    return True
...
async def _get_original_order(cash_sale):
    ...
    if original_order_cf_value:
        cash_sale = get_record_by_type('cashSale', original_order_cf_value.internalId)
...
def get_or_create_customer(order_data):
    ...
    netsuite_customer_internal_id = nsac.lookup_customer_id_by_name_and_email(customer)
...
def search_sales_order(order_id, external_order_id):
    sales_order = get_sales_order(order_id)
    if not sales_order:
        LOGGER.info(f'Search for SalesOrder using order id, {order_id}, not found, try with external order id.')
        # When partner is Dell Boomi, the search uses the external order id
        sales_order = get_sales_order(external_order_id)
        if not sales_order:
            LOGGER.info(f'Search for SalesOrder using external order id, {external_order_id} not found, try with custom field order number.')
...
def is_historical_return(original_payment, return_id):
    if original_payment['reason'] == 'refund' \
        ...
        extended_attributes = original_payment.get('metadata', {}).get('extended_attributes', [])
        is_historical = dict(next((attribute for attribute in extended_attributes if attribute['name'] == 'is_historical'), {}))

        if is_historical.get('value', 'False').lower() in ('1', 'yes', 'true', 1):
            LOGGER.info('This is a historical return and will not be processed. Discarding....')
            return True
