...
async def map_cash_refund_items(customer_order, ns_return, _, location_id=None, credit_memo_init_item_list=None):
    ...
    for item in customer_order['products']:
        ...
        if item['status'] != 'returned' \
                ...
                or (credit_memo_init_item_list and not credit_memo_init_item):
        ...
        netsuite_item_id = get_netsuite_item_id(
            item, item_custom_field_list, customer_order)
        ...
        if credit_memo_init_item:
            cash_refund_item['rate'] = str(item['price_gross'])
        ...
        if location_id is not None:
            ...
            if not is_sellable:
                LOGGER.info(
                    "Item cannot be resold, sending to store's damage location")
            else:
                sellable.remove(item['id'])
            item_location_id = Utils.get_netsuite_store_internal_id(
                return_location_id, is_sellable)
            if not item_location_id:
                raise Exception(
                    f"Returned_from store {return_location_id} isn't mapped to NetSuite")
            cash_refund_item['location'] = RecordRef(
                internalId=item_location_id)
        ...
        if item_custom_field_list:
            cash_refund_item['customFieldList'] = CustomFieldList(
                item_custom_field_list)
    ...
    if Utils.is_product_gift_card(item['id']):
        ...
        if gc_number:
            item_custom_field_list.append(StringCustomFieldRef(
                scriptId='custcol_nws_gcnumber', value=gc_number))
        else:
            LOGGER.info(
                'Serial Number for gift card not provided on the external fields.')
    else:
        ...
        if netsuite_item_id:
            ...
            raise Exception(
                f"Unable to fetch product internal ID from NetSuite. Product {item['id']}")
...
def _get_external_identifiers(external_identifiers, key):
    result = next(
        (item for item in external_identifiers if item['type'] == key), False)
...
def map_custom_fields(customer_order):
    ...
    shopify_internal_id = Utils.get_extended_attribute(
