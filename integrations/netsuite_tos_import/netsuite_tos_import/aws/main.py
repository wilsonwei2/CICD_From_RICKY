...
TRANSFER_ORDER_SAVED_SEARCH_ID = Utils.get_netsuite_config()['transfer_orders_saved_search_id']
NETSUITE_SYNCED_TO_NEWSTORE_FLAG_SCRIPT_ID = Utils.get_netsuite_config()['transfer_orders_processed_flag_script_id']
...
async def import_transfer_orders():
    ...
    LOGGER.info(f"Found a total of {search_result['body']['searchResult']['totalRecords']} transfer orders")
    ...
    for transfer_order_payload in newstore_transfer_order_list:
        try:
            ...
            if 'order_already_exists' in str(nws_exc):
                LOGGER.warning(f'Transfer Order already exists in NOM, marking transfer as processed in NetSuite.')
                mark_to_imported_by_newstore(transfer_order_payload['internal_id'])
            else:
                LOGGER.error(f'Failed to create Transfer Order: {str(nws_exc)}', exc_info=True)
        else:
            mark_to_imported_by_newstore(transfer_order_payload['internal_id'])
            LOGGER.info(f"Transfer Order successfully injected {json.dumps(transfer_order_payload['newstore_transfer'], indent=4)}")
...
async def transform_search_result(search_result):
    ...
    for transfer_order_abstract in netsuite_transfer_order_list:
        LOGGER.info(f"Processing transfer order: {transfer_order_abstract['basic']['tranId'][0]['searchValue']}")
        is_success, transfer_order = await get_transfer_order(transfer_order_abstract['basic']['tranId'][0]['searchValue'])

        if not is_success:
            LOGGER.error(f'Failed to retrieve transfer order: {transfer_order}')
            continue

        transfer_order_payload = await transform_transfer(transfer_order)
        LOGGER.debug(f'Transfer Order Payload: {json.dumps(transfer_order_payload, indent=4)}')
...
async def transform_transfer(netsuite_transfer_order):
    destination = get_location_from_netsuite_id(netsuite_transfer_order['transferLocation']['internalId'])
    if not destination:
        destination = get_location_from_name(netsuite_transfer_order['transferLocation']['name'])
        if not destination:
    ...
    from_location = get_location_from_netsuite_id(netsuite_transfer_order['location']['internalId'])
    if not from_location:
        from_location = get_location_from_name(netsuite_transfer_order['location']['name'])
    ...
    return {
        ...
        "internal_id": netsuite_transfer_order['internalId']  # We need this to mark the order synced in NetSuite
    }
...
def mark_to_imported_by_newstore(internal_id):
    ...
    if response.body.writeResponse.status.isSuccess:
        LOGGER.info(f'Marked transfer order with id "{internal_id}" as imported by NewStore in Netsuite.')
    else:
        LOGGER.error(f'Failed to mark transfer order {internal_id} as synced: {response.body}')
