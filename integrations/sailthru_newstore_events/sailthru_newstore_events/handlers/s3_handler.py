...
class S3Handler:
    ...
    def __init__(self, record=None, bucket_name=None, key_name=None, profile_name=None):
        self.s3 = None # pylint: disable=invalid-name
    ...
    def getS3(self): # pylint: disable=invalid-name
        if self.s3 is None:
            self.s3 = self.session.client('s3')
        return self.s3

    def getS3BucketName(self): # pylint: disable=invalid-name
        return self.bucket_name

    def getS3Bucket(self): # pylint: disable=invalid-name
        return self.bucket

    def getS3BucketKey(self): # pylint: disable=invalid-name
        return self.bucket_key

    def getS3Resource(self): # pylint: disable=invalid-name
        if self.s3_resource is None:
            self.s3_resource = self.session.resource('s3')
        return self.s3_resource

    def getMultiPartObject(self): # pylint: disable=invalid-name
        return self.part_object

    def load_s3_Object(self): # pylint: disable=invalid-name
        if self.object is None:
            self.object = self.getS3Resource().Object(self.bucket_name, self.bucket_key)

    def getKeySize(self): # pylint: disable=invalid-name
    ...
    def getFiles(self): # pylint: disable=invalid-name
        LOGGER.info(f'Get File for Key --> {self.getS3BucketKey()}')

        obj = self.getS3Resource().Object(self.getS3BucketName(), self.getS3BucketKey())

        if self.getS3BucketKey().endswith('.zip'):
            with io.BytesIO(obj.get()["Body"].read()) as tf: # pylint: disable=invalid-name
                # rewind the file
                tf.seek(0)
                # Read the file as a zipfile and process the members
                with zipfile.ZipFile(tf, mode='r') as zf: # pylint: disable=invalid-name
                    return {name: zf.read(name) for name in zf.namelist()}
        else:
            return {self.getS3BucketKey().split("/")[-1]: obj.get()['Body'].read()}

    def getS3File(self): # pylint: disable=invalid-name
        """
    ...
    def get_json_files_dict(self):
        jsons = {}
        s3 = self.getS3Resource() # pylint: disable=invalid-name
