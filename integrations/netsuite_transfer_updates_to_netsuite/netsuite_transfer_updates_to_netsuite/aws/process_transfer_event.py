...
async def process_inventory_adj(message):
    ...
    LOGGER.info(f'TransferOrder: \n{json.dumps(transfer_order, indent=4, default=Utils.json_serial)}')
    item_fulfillment = netsuite_sale.initialize_record('itemFulfillment', 'transferOrder', transfer_order['internalId'])
    ...
    LOGGER.info(f'Initialized ItemFulfillment: \n{json.dumps(item_fulfillment, indent=4, default=Utils.json_serial)}')

    store, products_info = get_store_and_products_from_nom(asn_created_event['items'], asn_created_event['origin_id'])
    item_fulfillment_transformed = pif.transform_item_fulfillment(asn_created_event, products_info, store, item_fulfillment)
    LOGGER.info(f'Transformed ItemFulfillment: \n{json.dumps(serialize_object(item_fulfillment_transformed), indent=4, default=Utils.json_serial)}')

    result = netsuite_sale.create_item_fulfillment(item_fulfillment_transformed)
    if result:
        LOGGER.info(f'Item Fulfillment successfully created in NetSuite internal ID {result}')
        Utils.mark_event_as_received(f"{asn_created_event['id']}-{asn_created_event['chunk_number']}", DYNAMO_TABLE)
    else:
...
def get_store_and_products_from_nom(items, fulfillment_node_id):
    ...
    for product_id in product_ids:
        product = Utils.get_product(product_id, store.get('catalog', 'storefront-catalog-en'), store.get('locale', 'en-US'))
